...
\section{Dodatkowe wymagania}
	\begin{step}
		...
		\item Korzystanie z aplikacji wymaga konta w uczelnianym Centralnym Systemie Uwierzytelniania (CAS, z ang. texit{Central Authentication Service}).
	\end{step}
...
\subsection{Słownik uzasadnień}
Widok Słownika uzasadnień jest bardzo prosty w budowie i obsłudze. Na ekranie \ref{fig:uzasadnienia} widać jednostki, typy, nazwy po polsku, nazwy po angielsku uzasadnień. Po wybraniu uzasadnienia można zobaczyć treść po polsku i treść po angielsku na dole ekranu.
...
\subitem{Akcja odpowiada za wyświetlanie jednostek i informacji o liczbie spotkań w każdej z jednostek. Przyjmuje dodatkowy parametr w GET, który włącza lub ~wyłącza ograniczenie listy jednostek tylko do tych, które są związane ze~studentem.}}
...
\item{MojeSpotkania}
\subitem{Lista spotkań związanych ze studentem. Widok jest identyczny z~widokiem spotkań jednostki, ale pokazywane są tylko spotkania i~kalendarze związane z danym studentem, czyli takie spotkania, na które on jest zapisany. Nie ma żadnych ograniczeń na jednostkę, w której dane spotkanie się odbywa.}
\item{Spotkanie}
\subitem{Szablon pomocniczy używany w szablonie FormularzSpotkania.}
\item{WyborJednostki}
\subitem{Lista jednostek z informacją o liczbie kalendarzy spotkań w danej jednostce.}
\end{itemize}
...
\item{SpotkanieOsoba}
\subitem{Klasa jest modelem opisującym stan spotkania w~kontekście danej osoby. Ma dużą liczbę niestatycznych funkcji pomocniczych, wykorzystywanych w wielu miejscach modułu spotkań.}
\item{SpotkaniaUtils}
...
\begin{figure}[!]
  ...
  \label{fig:zapisweb}
\end{figure} % TODO podmienic obrazek
...
\item{trzeci poziom}
\subitem{spotkania (terminy) z informacją o stanie danego spotkania w kontekście zalogowanego studenta (stan zapisu, stan spotkania, liczba osób zapisanych na~listę~główną oraz~rezerwową, data i~czas rozpoczęcia i~zakończenia spotkania).}
\end{itemize}
